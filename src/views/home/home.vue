  ...
  <a-layout>
    ...
    <a-layout>
      <a-layout-sider theme="light" width="50%" class="height: 100%">
        <a-layout>
          <a-layout-sider theme="light" width="40%" class="height: 100%">
            <!-- <a-row class="top-20">
              <a-col :span="24">
            ...
            <a-col class="top-20" :span="20" :offset="2">
            <a-table :columns="topNDataColumns" :data-source="topNDataTable" :bordered="false" :pagination="false">
              <template #name="{ text }">
                <a>{{ text }}</a>
              </template>
              <template #bodyCell="{ index, text }">
                <template v-if="index == topNowIndex">
                  <a-typography-text type="danger">{{text}}</a-typography-text>
                </template>
              </template>
            </a-table>
            </a-col>
        ...
        <a-layout-content>
          <h3 :style="{ marginBottom: '16px', marginTop: '16px' }"> 用户：{{user.name}}，总市值：{{user.totalMoney}}，可用余额：{{user.money}}</h3>
          <a-col :span="20" :offset="2">
            <!-- <a-list :grid="{ gutter: 6, xs: 1, sm: 1, md: 1, lg: 1, xl: 1, xxl: 1 }" item-layout="horizontal" size="small" bordered :data-source="user.stocks">
              ...
              <template #renderItem="{ item }">
                <a-list-item>{{ item.Symbol }} - (可用/持仓)：{{item.StockNum}}/{{ item.StockNum + item.BandStockNum }}</a-list-item>
              </template>
            </a-list> -->
            <a-col class="top-20">
              <a-table :columns="ccDataColumns" :data-source="user.stocks" :bordered="false" :pagination="false">
                <template #name="{ text }">
                  <a>{{ text }}</a>
                </template>
                <template #bodyCell="{ column, record }">
                  <template v-if="column.key === 'value'">
                    <span>{{ Math.round(((record.StockNum + record.BandStockNum) * record.Cur_price) * 100) / 100 }}</span>
                  </template>

                  <template v-if="column.key === 'add'">
                    <span>{{ Math.round((parseFloat(record.Cur_price) - record.Cost) * (record.BandStockNum + record.StockNum) * 100) / 100 }}</span>
                  </template>

                  <template v-if="column.key === 'amount'">
                    <span>{{ (record.BandStockNum + record.StockNum) + "/" + (record.StockNum)}}</span>
                  </template>

                  <template v-if="column.key === 'price'">
                    <span>{{ Math.round(record.Cost * 100) / 100 + "/" + Math.round(record.Cur_price * 100) / 100 }}</span>
                  </template>
...
export default defineComponent({
  setup() {
    ...
    const onChangeSymbol = (event)=>{
      if(event) symbol.value = event.target.value;
      formState.symbol = symbol.value;
      Init();
    };
    ...
    const reViewTopN = () => {
      ...
      wsClient.sendCustomMsg(msg, (res) => {
        if(res.code == 0) {
          ...
          topNowIndex.value = topNDataTable.value.length
          topNDataTable.value.push({amount:"", price:res.data.now, name:"最新价"})

          // 存入最新价
          nowPrice[symbol.value] = res.data.now;
        ...
        if(res.code == 0) {
          ...
          if(res.data.Stocks) {
            ...
            for (let i in res.data.Stocks) {
              ...
              if(!nowPrice[res.data.Stocks[i].Symbol]) {
                nowPrice[res.data.Stocks[i].Symbol] = 0;
              }
            }
          }          
        } else {
